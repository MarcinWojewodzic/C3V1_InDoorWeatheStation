...
void GFX_DrawRectangle(int x, int y, uint16_t w, uint16_t h, uint8_t color, Screen_TypeDef Screen)
{
   ...
   GFX_DrawFastHLine(x, y, w, color, Screen);
   GFX_DrawFastHLine(x, y + h - 1, w, color, Screen);
   GFX_DrawFastVLine(x, y, h, color, Screen);
...
void GFX_DrawCircle(int x0, int y0, uint16_t r, uint8_t color, Screen_TypeDef Screen)
{
   ...
   GFX_DrawPixelScreen(x0, y0 + r, color, Screen);
   GFX_DrawPixelScreen(x0, y0 - r, color, Screen);
   GFX_DrawPixelScreen(x0 + r, y0, color, Screen);
   GFX_DrawPixelScreen(x0 - r, y0, color, Screen);
   ...
   while(x < y)
   {
      ...
      GFX_DrawPixelScreen(x0 + x, y0 + y, color, Screen);
      GFX_DrawPixelScreen(x0 - x, y0 + y, color, Screen);
      GFX_DrawPixelScreen(x0 + x, y0 - y, color, Screen);
      GFX_DrawPixelScreen(x0 - x, y0 - y, color, Screen);
      GFX_DrawPixelScreen(x0 + y, y0 + x, color, Screen);
      GFX_DrawPixelScreen(x0 - y, y0 + x, color, Screen);
      GFX_DrawPixelScreen(x0 + y, y0 - x, color, Screen);
      GFX_DrawPixelScreen(x0 - y, y0 - x, color, Screen);
   }
...
#endif
#ifdef CIRCLE_HELPER
void GFX_DrawCircleHelper(int x0, int y0, uint16_t r, uint8_t cornername, uint8_t color, Screen_TypeDef Screen)
{
   int16_t f     = 1 - r;
   int16_t ddF_x = 1;
   int16_t ddF_y = -2 * r;
...
void GFX_DrawFillCircleHelper(int x0, int y0, uint16_t r, uint8_t cornername, int16_t delta, uint8_t color, Screen_TypeDef Screen)
{
   ...
   while(x < y)
   {
      ...
      if(cornername & 0x1)
      {
         GFX_DrawFastVLine(x0 + x, y0 - y, 2 * y + 1 + delta, color, Screen);
         GFX_DrawFastVLine(x0 + y, y0 - x, 2 * x + 1 + delta, color, Screen);
      }
      if(cornername & 0x2)
      {
         GFX_DrawFastVLine(x0 - x, y0 - y, 2 * y + 1 + delta, color, Screen);
...
void GFX_DrawFillCircle(int x0, int y0, uint16_t r, uint8_t color, Screen_TypeDef Screen)
{
   ...
   GFX_DrawFastVLine(x0, y0 - r, 2 * r + 1, color, Screen);
   GFX_DrawFillCircleHelper(x0, y0, r, 3, 0, color, Screen);
}
#endif
...
{
   GFX_DrawFastHLine(x + r, y, w - 2 * r, color, Screen);         // Top
   GFX_DrawFastHLine(x + r, y + h - 1, w - 2 * r, color, Screen); // Bottom
   GFX_DrawFastVLine(x, y + r, h - 2 * r, color, Screen);         // Left
   GFX_DrawFastVLine(x + w - 1, y + r, h - 2 * r, color, Screen); // Right
   // draw four corners
   GFX_DrawCircleHelper(x + r, y + r, r, 1, color, Screen);
   GFX_DrawCircleHelper(x + w - r - 1, y + r, r, 2, color, Screen);
   GFX_DrawCircleHelper(x + w - r - 1, y + h - r - 1, r, 4, color, Screen);
   GFX_DrawCircleHelper(x + r, y + h - r - 1, r, 8, color, Screen);
}
#endif
...
void GFX_DrawFillRoundRectangle(int x, int y, uint16_t w, uint16_t h, uint16_t r, uint8_t color, Screen_TypeDef Screen)
{
   ...
   GFX_DrawFillRectangle(x + r, y, w - 2 * r, h, color, Screen);

   // draw four corners
   GFX_DrawFillCircleHelper(x + w - r - 1, y + r, r, 1, h - 2 * r - 1, color, Screen);
   GFX_DrawFillCircleHelper(x + r, y + r, r, 2, h - 2 * r - 1, color, Screen);
}
#endif
#if USING_TRIANGLE == 1
void GFX_DrawTriangle(int x0, int y0, int x1, int y1, int x2, int y2, uint8_t color, Screen_TypeDef Screen)
{
   GFX_DrawLine(x0, y0, x1, y1, color, Screen);
   GFX_DrawLine(x1, y1, x2, y2, color, Screen);
   GFX_DrawLine(x2, y2, x0, y0, color, Screen);
}
#endif
...
void GFX_DrawFillTriangle(int x0, int y0, int x1, int y1, int x2, int y2, uint8_t color, Screen_TypeDef Screen)
{
   ...
   { // Handle awkward all-on-same-line case as its own thing
      ...
      if(x2 < a)
         ...
         b = x2;
      GFX_DrawFastHLine(a, y0, b - a + 1, color, Screen);
      return;
   }
   ...
   for(y = y0; y <= last; y++)
   {
      ...
      */
      if(a > b)
         _swap_int(a, b);
      GFX_DrawFastHLine(a, y, b - a + 1, color, Screen);
   }
   ...
   for(j = 0; j < h; j++)
   {
      for(i = 0; i < w; i++)
      {
         if(pgm_read_byte(img + j * byteWidth + i / 8) & (128 >> (i & 7)))
            GFX_DrawPixelScreen(x + i, y + j, color, Screen);
      }
   }
}
#endif
...
const double sinus_LUT[] = {
   // 0* to 90* = 91 valuse
   0.0000, 0.0175, 0.0349, 0.0523, 0.0698, 0.0872, 0.1045, 0.1219, 0.1392, 0.1564, 0.1736, 0.1908, 0.2079, 0.2250, 0.2419, 0.2588,
   0.2756, 0.2924, 0.3090, 0.3256, 0.3420, 0.3584, 0.3746, 0.3907, 0.4067, 0.4226, 0.4384, 0.4540, 0.4695, 0.4848, 0.5000, 0.5150,
   0.5299, 0.5446, 0.5592, 0.5736, 0.5878, 0.6018, 0.6157, 0.6293, 0.6428, 0.6561, 0.6691, 0.6820, 0.6947, 0.7071, 0.7193, 0.7314,
   0.7431, 0.7547, 0.7660, 0.7771, 0.7880, 0.7986, 0.8090, 0.8192, 0.8290, 0.8387, 0.8480, 0.8572, 0.8660, 0.8746, 0.8829, 0.8910,
   0.8988, 0.9063, 0.9135, 0.9205, 0.9272, 0.9336, 0.9397, 0.9455, 0.9511, 0.9563, 0.9613, 0.9659, 0.9703, 0.9744, 0.9781, 0.9816,
   ...
   for(j = 0; j < h; j++)
   {
      for(i = 0; i < w; i++)
      {
         if(img[j * byteWidth + i / 8] & (128 >> (i & 7)))
         {
            ...
            GFX_DrawPixelScreen(xs + x, ys + y, color, Screen);
         }
      }
   }
}
